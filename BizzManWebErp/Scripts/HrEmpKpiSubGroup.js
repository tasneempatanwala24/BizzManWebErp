...
function BindMasterList() {
    $.ajax({
        type: "POST",
        url: 'wfHrEmpKpiSubGroup.aspx/FetchMasterList',
        data: {},
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        beforeSend: function () {
        ...
        success: function (response) {
            ...
            for (var i = 0; i < JSON.parse(response.d).length; i++) {
                html = html + '<tr onclick="FetchMasterDetails(\'' + data[i].Id + '\')"><td>' + data[i].Id + '</td>'
                    ...
                    + '<td>' + data[i].KpiSubGroupName + '</td></tr>';
            }
...
function FetchMasterDetails(id) {
    $.ajax({
        type: "POST",
        url: 'wfHrEmpKpiSubGroup.aspx/FetchMasterDetails',
        data: JSON.stringify({
            "Id": id
        }),
        contentType: "application/json; charset=utf-8",
        ...
        //    $('#divEmpJobList').hide();
         ...
         //   $('#divEmpJobEntry').show();
            ...
            $('#txtSubGroupName').val(data[0].KpiSubGroupName);
            $('#ddlKpiGroup').val(data[0].KpiGroupId);
            $('#txtId').val(data[0].Id);
...
function AddData() {
    ...
    if (isValid) {
        $.ajax({
            type: "POST",
            url: 'wfHrEmpKpiSubGroup.aspx/CheckDataAvailability',
            ...
            success: function (response) {
                ...
                if (data == 'False') {
                    $.ajax({
                        type: "POST",
                        url: 'wfHrEmpKpiSubGroup.aspx/AddData',
                        data: JSON.stringify({
